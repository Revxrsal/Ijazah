...
export const DynamicField = (props: {
  ...
  return (
    <div class={`mb-2`} style={{"margin-inline-start": `${props.nesting * 4}px`}}>
      <Switch>
        <Match when={metadata.type === "string"}>
          <StringField value={props.value} onUpdate={props.onUpdate} key={props.key} metadata={metadata as StringPropertyMetadata}/>
        </Match>

        <Match when={metadata.type == "decimal"}>
          <DecimalField value={props.value} onUpdate={props.onUpdate} key={props.key} metadata={metadata as DecimalPropertyMetadata}/>
        </Match>
        ...
        <Match when={metadata.type === "boolean"}>
          <BooleanField value={props.value} onUpdate={props.onUpdate} key={props.key} metadata={metadata as BooleanPropertyMetadata}/>
        </Match>

        <Match when={metadata.type === "enum"}>
          <EnumField value={props.value} onUpdate={props.onUpdate} key={props.key} metadata={metadata as EnumPropertyMetadata}/>
        </Match>
